...
hx_upgrade_module module_list[] = {
	{
		.lcd_name = "lcd_hx83102e_inx_mipi_hdp",
		.fw_num = MODEL_INX,
	},
	{
	...
	{
		.lcd_name = "lcd_hx83102e_ls_hsd_mipi_hdp",
		.fw_num = MODEL_LS_HSD,
	},
	{
		.lcd_name = "lcd_hx83102e_xy_inx_mipi_hdp",
		.fw_num = MODEL_XY_INX,
	},
};
...
static int himax_get_tp_module(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(module_list); i++) {
		if (NULL != strstr(command_line, module_list[i].lcd_name)) {
			return module_list[i].fw_num;
		}
	}

	E("%s: Not find tp module !!\n", __func__);
...
static void himax_update_module_info(void)
{
    ...
    case MODEL_INX:
            ...
            ts->firmware_filp_open.data = CTPM_FW_INX;
            ts->firmware_filp_open.size = sizeof(CTPM_FW_INX);
#endif
            break;
    case MODEL_XY_HSD:
...
#if defined(FW_GET_FROM_I_FILE)
            ts->firmware_filp_open.data = CTPM_FW_XY_HSD;
            ts->firmware_filp_open.size = sizeof(CTPM_FW_XY_HSD);
#endif
            break;
	case MODEL_LS_HSD:
...
#if defined(FW_GET_FROM_I_FILE)
            ts->firmware_filp_open.data = CTPM_FW_LS_HSD;
            ts->firmware_filp_open.size = sizeof(CTPM_FW_LS_HSD);
#endif
            break;
	/*Tab A8 code for SR-AX6300-01-388 by huangzhongjie at 2021/09/16 start*/
	case MODEL_XY_INX:
            strcpy(ts->himax_name,"hx83102e_xy_inx");
            strcpy(ts->himax_nomalfw_rq_name, "hx83102e_xy_inx_firmware.bin");
            strcpy(ts->himax_mpfw_rq_name, "hx83102e_xy_inx_mpfw.bin");
            strcpy(ts->himax_csv_name, "hx_xy_inx_criteria.csv");
#if defined(FW_GET_FROM_I_FILE)
            ts->firmware_filp_open.data = CTPM_FW_XY_INX;
            ts->firmware_filp_open.size = sizeof(CTPM_FW_XY_INX);
#endif
            break;
	/*Tab A8 code for SR-AX6300-01-388 by huangzhongjie at 2021/09/16 end*/
    default:
...
static int __init himax_common_init(void)
{
	...
	/*Tab A8 code for AX6300DEV-805 by suyurui at 2021/10/13 end*/
	if (g_mmi_refcnt++ > 0) {
