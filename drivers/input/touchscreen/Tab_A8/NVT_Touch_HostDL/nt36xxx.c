...
void nvt_bootloader_reset(void)
{
	...
	/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 start*/
	mdelay(20);	//wait tBRST2FR after Bootload RST
	/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 end*/
	if (SPI_RD_FAST_ADDR) {
		/* disable SPI_RD_FAST */
...
int32_t nvt_get_fw_info(void)
{
	...
	/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 start*/
	buf = (uint8_t *)kzalloc(64, GFP_KERNEL | GFP_DMA);
	if(buf == NULL) {
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		nvt_read_fw_history(0x39700);
		nvt_read_fw_history(0x39740);
...
*/
	...
	if (nvt_wdt_fw_recovery(point_data)) {
		...
		/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 start*/
		goto XFER_ERROR;
	} else if ((recovery_cnt > 0) && (point_data[1] == 0xFD)) {
		input_info(true, &ts->client->dev, "All 0xFD packet from IC (%d)\n", recovery_cnt);
		goto XFER_ERROR;
	} else if ((recovery_cnt > 0) && (point_data[1] == 0xFE)) {
		input_info(true, &ts->client->dev, "All 0xFE packet from IC (%d)\n", recovery_cnt);
		/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 end*/
		goto XFER_ERROR;
	}
#endif /* #if NVT_TOUCH_WDT_RECOVERY */
	...
	for (i = 0, finger_cnt = 0; i < platdata->max_touch_num; i++) {
		...
		if ((status == FINGER_ENTER) || (status == FINGER_MOVING) || (status == GLOVE_TOUCH) || (status == PALM_TOUCH) || (status == METAL_JIG_TOUCH)) {
			...
			/*Tab A8 code for AX6300DEV-963 AX6300DEV-964 by luxinjun at 2021/09/27 start*/
			//input_report_abs(ts->input_dev, ABS_MT_CUSTOM, (palm | metal_jig));
			input_report_key(ts->input_dev, BTN_PALM, (palm | metal_jig));
			/*Tab A8 code for AX6300DEV-963 AX6300DEV-964 by luxinjun at 2021/09/27 end*/
...
/*Tab A8 code for SR-AX6300-01-373 by luxinjun at 2021/09/10 start*/
#if 0
static ssize_t nvt_ts_suspend_show(struct device  *dev,
...
/* reset chip */
static ssize_t nvt_ts_suspend_store(struct device *dev,
...
static DEVICE_ATTR(ts_suspend, S_IRUGO | S_IWUSR | S_IWGRP, nvt_ts_suspend_show, nvt_ts_suspend_store);
...
static void nvt_charger_notify_work(struct work_struct *work)
{
	...
	/*Tab A8 code for AX6300DEV-1320 by luxinjun at 2021/10/12 start*/
	if (mutex_lock_interruptible(&ts->lock))
		return;
	/*Tab A8 code for AX6300DEV-1320 by luxinjun at 2021/10/12 end*/
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 start*/
	point_data = kzalloc((POINT_DATA_LEN + 1 + DUMMY_BYTES), GFP_KERNEL | GFP_DMA);
	if (point_data == NULL) {
		input_err(true, &client->dev, "failed to allocated memory for point_data\n");
		ret = -ENOMEM;
		goto err_malloc_point_data;
	}
	/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 end*/
...
//	if (ts->platdata->support_ear_detect)
	...
	if (ts->nvt_resume_wq)
		INIT_DELAYED_WORK(&ts->nvt_resume_work, resume_work);
...
int nvt_suspend_by_esd(void)
{
	nvt_ts_suspend(&ts->client->dev);
	input_info(true, &ts->client->dev, "Touch is suspend by esd!\n");
	return 0;
}
EXPORT_SYMBOL(nvt_suspend_by_esd);

int nvt_resume_by_esd(void)
{
	nvt_ts_resume(&ts->client->dev);
	input_info(true, &ts->client->dev, "Touch is resume by esd!\n");
	return 0;
}
EXPORT_SYMBOL(nvt_resume_by_esd);
/*******************************************************
Description:
...
#if NVT_TOUCH_ESD_PROTECT
/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 start*/
// comment below disable to keep esd check alive after FW update
//	nvt_esd_check_enable(false);
/*Tab A8 code for AX6300DEV-1956 by huangzhongjie at 2021/11/04 end*/
	queue_delayed_work(nvt_esd_check_wq, &nvt_esd_check_work,
			msecs_to_jiffies(NVT_TOUCH_ESD_CHECK_PERIOD));
...
void nvt_resume_for_earlier(void)
{
	...
	queue_delayed_work(ts->nvt_resume_wq, & ts->nvt_resume_work, msecs_to_jiffies(30));
}
/*Tab A8 code for AX6300DEV-1494 by luxinjun at 2021/11/03 end*/
...
static int nvt_drm_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	input_info(true, &ts->client->dev,"event = %d DISPC_POWER_OFF = %d DISPC_POWER_ON = %d\n", event, DISPC_POWER_OFF, DISPC_POWER_ON);
		...
		cancel_delayed_work_sync(&ts->nvt_resume_work);
		nvt_ts_suspend(&ts->client->dev);
        } else if (event == DISPC_POWER_ON) {
		input_info(true, &ts->client->dev, "event=%lu\n", event);
	//	nvt_ts_resume(&ts->client->dev);
		queue_delayed_work(ts->nvt_resume_wq, & ts->nvt_resume_work, msecs_to_jiffies(0));
	}
	return 0;
}
/*Tab A8 code for AX6300DEV-1494 by luxinjun at 2021/11/03 end*/
...
static struct of_device_id nvt_match_table[] = {
	{ .compatible = "novatek,NVT-ts-spi-a8",},
	{ },
};
#endif
