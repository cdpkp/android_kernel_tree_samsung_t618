...
int gcore_fw_event_notify(enum fw_event_type event)
{
    ...
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 202204010 start*/
    case FW_REPORT_MP_TEST:
        cmd[cmd_len - 1] = 0x12;
        GTP_DEBUG("gcore fw event:mp test");
        break;
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 202204010 end*/
    default:
...
int gcore_read_fw_version(u8 *version, int length)
{
    ...
    if (gdev_fwu->ic_type == IC_GC7202H) {
    ...
    fn_data.gdev->fw_ver = ( (version[1] << 24) |
                            (version[0] << 16) |
                            (version[3] <<  8) |
                            (version[2]) );
    usleep_range(1000, 1100);
...
void gcore_request_firmware_update_work(struct work_struct *work)
{
    ...
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 20220407 start*/
    strncpy(fw_bin_name, fn_data.gdev->gc_fw_name, sizeof(fw_bin_name));
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 20220407  end*/
    GTP_DEBUG("open fw bin file:%s", fw_bin_name);
    ...
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 20220409 start*/
    fn_data.gdev->fw_ver = ( (fw_buf[FW_VERSION_ADDR+1] << 24) |
                            (fw_buf[FW_VERSION_ADDR] << 16) |
                            (fw_buf[FW_VERSION_ADDR+3] <<  8) |
                            (fw_buf[FW_VERSION_ADDR+2]) );
    /*HS03 code for SR-SL6215-01-1189 by duanyaoming at 20220409 end*/
    if (fw) {
        release_firmware(fw);
    }
#else
    ...
    // msleep(10);
    // gcore_read_fw_version(fw_version,4);
    return;
